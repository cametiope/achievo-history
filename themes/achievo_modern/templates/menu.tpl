...
{foreach from=$menuitems item=menuitem}
  {if $menuitem.name!=='-'}
    <a href="#" onclick="showSubMenu('{$menuitem.name}'); window.open('{$menuitem.url}','main','');" onmouseover="this.style.cursor = 'pointer'">
      <div id="mi_{$menuitem.name}" class="menuItemLevel1">
        <span class="menu-menuitem">{atktext $menuitem.name module=$menuitem.module}</span>
      </div>
    </a>
  {/if}
        
  {if (count($menuitem.submenu)>0)}
    <div id="smi_{$menuitem.name}" style="display: none; background: url(../images/menuLevel2Bg.jpg) repeat-y top left; color: #333;">
      {foreach from=$menuitem.submenu item=submenuitem}
         {if $submenuitem.enable && $submenuitem.name!=='-'}
           <a class="menuItemLevel2" onclick="window.open('{$submenuitem.url}','main','')" onmouseover="this.style.cursor = 'pointer'; this.style.fontWeight = 'bold';" onmouseout="this.style.fontWeight = '';">
             {atktext $submenuitem.name module=$submenu.module}
           </a>
         {/if}
      {/foreach}
    </div>
